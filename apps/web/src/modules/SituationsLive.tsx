...
import { situationNet } from "../lib/calc";
...
export default function SituationsLive() {
  ...
  const [f, setF] = useState({ chantier: "", cumulPct: "", montantHT: "", statut: "Brouillon" });
  ...
  async function create(e: React.FormEvent) {
    ...
    const { data: numero, error: ne } = await supabase.rpc("next_numero", { kind: "situation" });
    if (ne) return setErr(ne.message);
    const { error } = await supabase.from("situations").insert({ id: crypto.randomUUID(), tenantId: tid, chantier: f.chantier, numero: numero as string, cumulPct: Number(f.cumulPct) || 0, montantHT: Number(f.montantHT) || 0, retenuePct: 10, statut: f.statut, createdAt: new Date().toISOString() });
    if (error) return setErr(error.message.includes("row-level") ? "Droits insuffisants (rôle)." : error.message);
    setF({ chantier: "", cumulPct: "", montantHT: "", statut: "Brouillon" }); void load();
  }
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      ...
      <Card>
        <form onSubmit={create} style={{ display: "grid", gridTemplateColumns: "1.8fr 0.9fr 1.4fr 1.2fr auto", gap: 10 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {rows.map((s) => {
                ...
                const { retenue: ret } = situationNet(ht, Number(s.retenuePct));
                return (
